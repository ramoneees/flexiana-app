...
* [`src/cljs/flexiana_frontend/`](src/cljs/flexiana_frontend/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/flexiana_frontend/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `flexiana-frontend` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/flexiana_frontend/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns flexiana-frontend.example
  (:require [flexiana-frontend.config :as config])
